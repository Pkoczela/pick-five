...
export async function POST(request: NextRequest) {
  ...
  const userClient=await createUserClient(); const {data:{user}}=await userClient.auth.getUser(); const {data:member}=user?await userClient.from("league_members").select("role").eq("user_id",user.id).eq("active",true).limit(1).maybeSingle():{data:null};
