...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return redirectWith(request, "/leagues", "error", parsed.error.issues[0]?.message ?? "Invalid display name.");
  ...
  if (error) return redirectWith(request, "/leagues", "error", friendlyDisplayNameError(error.message));
  return redirectWith(request, "/leagues", "notice", "Your display name was updated.");
}
