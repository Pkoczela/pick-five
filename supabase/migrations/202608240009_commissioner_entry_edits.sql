...
declare v_week public.weeks;v_league_id uuid;v_entry_id uuid;v_valid_count integer;v_before jsonb;
begin
  ...
  select count(*) into v_valid_count from(
    select distinct x.game_id from jsonb_to_recordset(p_picks) as x(game_id uuid,selected_side text)
  ...
  select jsonb_build_object('entry',to_jsonb(e),'picks',(select jsonb_agg(to_jsonb(p)) from public.picks p where p.entry_id=e.id)) into v_before from public.entries e where e.week_id=p_week_id and e.league_member_id=p_member_id;
  insert into public.entries(week_id,league_member_id,status,tiebreaker_points,submitted_at,last_edited_at,commissioner_edited,commissioner_edit_reason)
  values(p_week_id,p_member_id,case when now()>=v_week.lock_at then 'LOCKED' else 'SUBMITTED' end,p_tiebreaker_points,now(),now(),true,p_reason)
  on conflict(week_id,league_member_id) do update set status=excluded.status,tiebreaker_points=excluded.tiebreaker_points,submitted_at=coalesce(public.entries.submitted_at,now()),last_edited_at=now(),commissioner_edited=true,commissioner_edit_reason=p_reason
  ...
  select v_entry_id,g.id,x.selected_side::public.pick_side,case when x.selected_side='HOME' then g.home_team_id else g.away_team_id end,l.id
  from jsonb_to_recordset(p_picks) as x(game_id uuid,selected_side text)
  join public.games g on g.id=x.game_id join public.official_lines l on l.game_id=g.id and l.is_current;
  insert into public.audit_events(league_id,actor_user_id,entity_type,entity_id,event_type,before_json,after_json,reason)
  values(v_league_id,auth.uid(),'entry',v_entry_id,'PLAYER_ENTRY_COMMISSIONER_EDITED',v_before,jsonb_build_object('week_id',p_week_id,'member_id',p_member_id,'tiebreaker',p_tiebreaker_points,'picks',p_picks),p_reason);
  return v_entry_id;
end;
$$;
